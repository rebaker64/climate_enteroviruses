...
for(i in 1:length(provlist)){
  ...
  sub <- sub[sub$WEEK !=53, ]
  subtsir <- sub[c("date","CVA16","births","Pop")]
  names(subtsir) <- c("time","cases","births","pop")
  plot(sub$date,sub$CVA16,type="l", main = sub$province[1])
  ...
  storerho <- rbind(storerho, mean(tsir$rho))
  sub$lcases <- lead(sub$CVA16)
  sub$empbeta <- (sub$lcases*sub$Pop)/((sub$CVA16^(0.975))*(tsir$sbar + tsir$Z))
...
finaldat$empbeta[finaldat$CVA16==0] <- NA

finaldat$cases <- finaldat$CVA16
