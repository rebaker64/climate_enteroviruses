...
coeffdat <- data.frame(model = rep(c("Polio","EVA71","CVA16"), each = 3), 
 ...
 title(xlab="Temperature coefficient", line = 2)
...
lm1eva <- felm(log(empbeta + 1) ~   temp + schlterm | province + year | 0| province, data =eva)
sumeva1 <- summary(lm1eva)
lm1cva <- felm(log(empbeta + 1) ~   temp + schlterm | province + year | 0| province, data =cva)
sumcva1 <- summary(lm1cva)

require("stargazer")
stargazer(lm1eva, lm1cva)
